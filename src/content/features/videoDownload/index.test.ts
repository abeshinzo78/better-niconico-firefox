...
import { apply, generateDownloadFilename } from './index';
...
describe('videoDownload/index', () => {
  ...
  describe('generateDownloadFilename', () => {
    it('should format filename as title-videoId.mp4', () => {
      expect(generateDownloadFilename('テスト動画', 'sm12345')).toBe('テスト動画-sm12345.mp4');
    });

    it('should sanitize filesystem-unsafe characters', () => {
      expect(generateDownloadFilename('タイトル/テスト:動画', 'sm12345')).toBe(
        'タイトル_テスト_動画-sm12345.mp4',
      );
    });

    it('should use videoId only when title is empty', () => {
      expect(generateDownloadFilename('', 'sm12345')).toBe('sm12345.mp4');
    });

    it('should strip "- ニコニコ動画" suffix from title', () => {
      expect(generateDownloadFilename('テスト動画 - ニコニコ動画', 'sm12345')).toBe(
        'テスト動画-sm12345.mp4',
      );
    });
  });
