 ...
 * ダウンロードファイル名を生成する
 * 形式: {タイトル}-{videoId}.mp4
 * タイトルが空の場合: {videoId}.mp4
 */
export function generateDownloadFilename(rawTitle: string, videoId: string): string {
  const title = rawTitle.replace(/ - ニコニコ動画$/, '').trim();
  const sanitized = sanitizeFilename(title);
  if (!sanitized) {
    return `${videoId}.mp4`;
  }
  return `${sanitized}-${videoId}.mp4`;
}

/**
 * 動画タイトルをページから取得（document.title から末尾の " - ニコニコ動画" を除去）
 */
...
async function handleDownload() {
  ...
  try {
    ...
    const filename = generateDownloadFilename(getVideoTitle(), videoId);
    ...
    // 5. Save MP4
    saveAsFile(muxedData, filename, 'video/mp4');
