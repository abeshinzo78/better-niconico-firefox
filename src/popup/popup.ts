...
function createSettingCard(config: SettingConfig): HTMLElement {
  ...
  if (config.icon) {
    ...
    iconContainer.className = 'setting-icon';
    ...
    settingHeader.appendChild(iconContainer);
  }

  const titleDiv = document.createElement('div');
  titleDiv.className = 'setting-title';
  titleDiv.textContent = config.label;
  settingHeader.appendChild(titleDiv);

  const descriptionDiv = document.createElement('div');
  descriptionDiv.className = 'setting-description';
  descriptionDiv.textContent = config.description;

  settingInfo.appendChild(settingHeader);
  settingInfo.appendChild(descriptionDiv);

  if (config.actionButton) {
    const actionBtn = document.createElement('button');
    actionBtn.className = 'manage-button';
    actionBtn.id = `action-${config.id}`;
    actionBtn.textContent = config.actionButton.label;
    actionBtn.addEventListener('click', config.actionButton.onClick);
    settingInfo.appendChild(actionBtn);
  }
...
function renderSettings() {
  ...
  if (filteredSettings.length === 0) {
    ...
    p.textContent = 'このカテゴリには設定がありません';
    emptyState.appendChild(p);
    settingsContainer.appendChild(emptyState);
    return;
  }
...
function renderTemplateList() {
  ...
  templates.forEach((template) => {
    ...
    previewDiv.textContent = template.comment;
    ...
    deletePath1.setAttribute('d', 'M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2');
